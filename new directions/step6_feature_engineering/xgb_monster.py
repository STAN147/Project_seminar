...
def main():
    ...
    print("="*100)
    print("XGBOOST: ПЛАТИНОВЫЙ ФОНД + ГИБРИДНАЯ НОРМАЛИЗАЦИЯ + ДИНАМИЧЕСКИЕ ПОРОГИ")
    print("="*100 + "\n")
    ...
    models = df_trans['Model'].unique()
    
    for test_model in models:
        # Трейн БЕЗ тестовой модели и БЕЗ COPA
        ...
        train_models_list = train_df['Model'].unique().tolist()
        ...
        for task in test_df['Task'].unique():
            ...
            # Вспомогательная функция для подсчета Hit Rate по динамическому числу слоев k
            ...
            # Сортируем предсказания по убыванию (чем выше скор, тем мусорнее слой)
            ...
            k_jump = np.argmax(diffs) + 1  # Индекс самого сильного обрыва
            ...
            drop_cluster_idx = np.argmax(centers) # Кластер с более высоким предсказанным скором
