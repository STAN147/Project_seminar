...
def setup_directories():
    paths = {}
    for task in TASKS:
        metric_dir = os.path.join(SCRIPT_DIR, "metrics", task)
        os.makedirs(metric_dir, exist_ok=True)
        paths[task] = {"metrics": metric_dir}
    return paths
    ...
    if task == "csqa":
        ...
        if os.path.exists(path):
            ...
            for _, row in df.iterrows():
                ...
                instruct_prompt = (
                    f"{raw_prompt}\n\n"
                    f"Instruction: Output ONLY the single correct letter (A, B, C, D, or E). "
                    f"Do not write any explanations, punctuation, or spaces.\nAnswer:"
                )
        ...
        if os.path.exists(path):
            ...
            for _, row in df.iterrows():
                ...
                instruct_prompt = (
                    ...
                    f"2) {ans2}\n"
                    f"3) {ans3}\n\n"
                    f"Instruction: Output ONLY the single correct digit (1, 2, or 3). "
                    f"Do not write any explanations, punctuation, or spaces.\nAnswer:"
                )
...
def compute_datafree_metrics(model, num_layers, compute_device):
    ...
    for layer_idx in tqdm(range(num_layers), desc="[Data-Free] M10, M11, M14, M15"):
        layer_signature = f".{layer_idx}."
        spectral_norms, frob_norms = [], []
        router_metrics = {
            "Router_Norm_Var": 0.0,
            "Router_Norm_Min": 0.0,
            "Router_Norm_Max": 0.0,
...
def run_task_other_metrics(model, tokenizer, task_name, samples):
    ...
    with torch.no_grad():
        for sample in tqdm(samples, desc=f"  [Скрытые состояния | {task_name.upper()}]"):
            inputs = tokenizer(sample["prompt"], return_tensors="pt", truncation=True, max_length=512).to(device)
    ...
    for i in range(num_layers):
        ...
        for j in range(num_layers + 1):
            ...
            r_m4[str(j)] = pearsonr(hi, hj)[0] if np.std(hi) > 0 and np.std(hj) > 0 else 0.0
            r_m6[str(j)] = np.linalg.norm(hi - hj, ord=np.inf)
            r_m7[str(j)] = np.var(hj) / (np.var(hi) + 1e-9)
            r_m8[str(j)] = pearsonr(hi, hj)[0] if np.std(hi) > 0 and np.std(hj) > 0 else 0.0
        m1_mse.append(r_m1)
        m4_cka.append(r_m4)
        m6_linf.append(r_m6)
        m7_var.append(r_m7)
    ...
    print(f"  [M19 Attn vs MLP Ratio | {task_name.upper()}]...")
    step_samples = samples[:500]
    ...
    for sample in tqdm(step_samples, desc=f"  [M19 Инференс | {task_name.upper()}]"):
        inputs = tokenizer(sample["prompt"], return_tensors="pt", truncation=True, max_length=512).to(device)
        with torch.no_grad():
...
def main():
    ...
    print(f"Модель TinyLlama успешно загружена. Количество слоев: {num_layers}")
    
    # 1. Расчет Data-Free метрик (M10, M11, M14, M15)
    ...
    # 2. Расчет матричных метрик, KL и M19
    for task in TASKS:
        samples = load_dataset(task)
        if samples:
            run_task_other_metrics(model, tokenizer, task, samples)
